...
*  File     AVMWebappLoader.java
*
*
*  NOTICE
*  ------
*          This code was derived from WebappLoader.java within Tomcat.
*          See: /apache-tomcat-5.5.17-src/container/catalina/src/
*                share/org/apache/catalina/loader/WebappLoader.java
*
*  The original Apache Software Foundation copyright notice is included below.
*----------------------------------------------------------------------------*/
 ...
 * @version $Revision: 303352 $ $Date: 2004-10-05 13:12:52 -0400 (Tue, 05 Oct 2004) $
 */

public class AVMWebappLoader
    implements Lifecycle, Loader, PropertyChangeListener, MBeanRegistration  
{

    /** @exclude
    *
    *     Stores association between AVMStandardContext name 
    *     and its associated classloader.  There is one 
    *     context_classloader_registry_ per AVMHostConfig.
    */
    Map<String, ClassLoader> context_classloader_registry_;

    /**
    * @exclude
    *
    *   The unique path name of the webapp being registered
    *   Example:  "/$-1$repo-3$my_webapp"
    *
    */
    ...
    * @exclude
    *
    *   The "parent" of the webapp being registered
    *   Example:  "/$-1$repo-2$my_webapp"
    *   <p>
    *   If webapp X overlays Y, then Y is the "parent" of X.
    */
    ...
    */
     ...
     */
    private AVMWebappClassLoader classLoader = null;


    /**
     * The Container with which this Loader has been associated.
     * (i.e.:  AVMStandardContext)
     */
    ...
    private static final String info =
        "org.alfresco.catalina.loader.AVMWebappLoader/1.0";
     ...
     * This class should extend AVMWebappClassLoader, otherwise, a different 
     * loader implementation must be used.
     */
    private String loaderClass =
        "org.alfresco.catalina.loader.AVMWebappClassLoader";
        ...
        if (reloadable && modified()) {
            try {
                Thread.currentThread().setContextClassLoader
                    (AVMWebappLoader.class.getClassLoader());
    ...
    public String toString() {
        ...
        StringBuffer sb = new StringBuffer("AVMWebappLoader[");
        if (container != null)
    ...
    public void start() throws LifecycleException {
        ...
        if (first) {
            ...
            try {
                ...
                // You can only call URL.setURLStreamHandlerFactory()
                // once per application.  When you have more than one
                // WebappLoader (e.g: non-AVMWebappLoader + AVMWebappLoader)
                // then setURLStreamHandlerFactory will be called more than
                // once, which is harmless... so skio the INFO.
                //
        ...
        try 
        {
            ...
            ObjectName cloname = new ObjectName
                (ctx.getEngineName() + ":type=AVMWebappClassLoader,path="
                 + path + ",host=" + ctx.getParent().getName());
    ...
    public void stop() throws LifecycleException 
    {
        ...
        // Make sure the associated context is no longer registered
        context_classloader_registry_.remove( context_path_ );
     ...
     */
    private AVMWebappClassLoader createClassLoader()
        throws Exception 
    {
        Class clazz = Class.forName(loaderClass);
        AVMWebappClassLoader classLoader = null;

        if (this.parentClassLoader == null)      // it won't be null for us
        {
            this.parentClassLoader = 
                Thread.currentThread().getContextClassLoader();
        }

        ClassLoader layered_parent_classloader = null;

        if ( parent_context_path_ != null )
        {
            layered_parent_classloader = 
                context_classloader_registry_.get( parent_context_path_ );
        }

        if ( layered_parent_classloader == null )
        {
            layered_parent_classloader = this.parentClassLoader;
        }

        Class[] argTypes = { ClassLoader.class };
        Object[] args = { layered_parent_classloader };
        Constructor constr = clazz.getConstructor(argTypes);
        classLoader = (AVMWebappClassLoader) constr.newInstance(args);

        // Register this classloader with the AVMHostConfig.
        //
        context_classloader_registry_.put( context_path_,
                                           classLoader);

        // jcox TODO:  mark dependent webapps as needing restart.
    ...
    private void setRepositories() {
        ...
        classLoader.setJarPath(libPath);     // see: AVMWebappClassLoader
        ...
        if (libDir != null) 
        {
            ...
            //         /opt/apache-tomcat-5.5.15/work/Catalina/
            //          avm.alfresco.localhost/$-1$repo-1$jcox.alfresco
            //   
            //    AVMWebappLoader libPath:
            //         /WEB-INF/lib
            //
    ...
    private static org.apache.commons.logging.Log log=
        org.apache.commons.logging.LogFactory.getLog( AVMWebappLoader.class );
