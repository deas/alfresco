   ...
   return declare([AlfDocumentPreviewPlugin], {
      ...
      display: function alfresco_preview_Image__display() {
         ...
         if (!this.attributes.src && srcMaxSize.match(/^\d+$/) && this.previewManager.size > parseInt(srcMaxSize))
         {
            ...
            var msg = '';
            msg += this.previewManager.msg("Image.tooLargeFile", this.previewManager.name, Alfresco.util.formatFileSize(this.previewManager.size));
            msg += '<br/>';
            msg += '<a class="theme-color-1" href="' + this.previewManager.getContentUrl(true) + '">';
            msg += this.previewManager.msg("Image.downloadLargeFile");
            msg += '</a>';
            msg += '<br/>';
            ...
            msg += this.previewManager.msg("Image.viewLargeFile");
            msg += '</a>';
         ...
         else
         {
            ...
            image.onerror = function()
            {
               ...
               this.previewManager.previewerNode.innerHTML = '<div class="message">'
                     + this.previewManager.msg("label.noPreview", this.previewManager.getContentUrl(true))
                     + '</div>';
            };
