   ...
   registerSuite({
      ...
      'Basic Test': function () {
         ...
         return TestCommon.bootstrapTest(this.remote, "./tests/alfresco/documentlibrary/views/page_models/FilmStripView_TestPage.json", testname)
            ...
            // Need to add in a fairly decent sleep to wait for the binary data to be loaded...
            .sleep(5000)
            .findAllByCssSelector(TestCommon.topicSelector("ALF_MOCK_XHR_SERVICE_READY", "publish", "any"))
               .then(function(elements) {
                  assert(elements.length === 1, "Timed out waiting for mock xhr data");
               })
               .end()
