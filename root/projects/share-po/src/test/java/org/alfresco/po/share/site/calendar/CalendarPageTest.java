...
public class CalendarPageTest extends AbstractSiteDashletTest {
	...
	public void testCreateEvent() {
		...
		calendarPage = calendarPage.createEvent(
				CalendarPage.ActionEventVia.MONTH_TAB, event1, event1, event1,
				null, null, null, null, tag1, false);
	}

	@Test(dependsOnMethods = "testCreateEvent", timeOut = 60000)
	public void testIsEventPresent() {
		Assert.assertTrue(calendarPage.isEventPresent(
				CalendarPage.EventType.MONTH_TAB_SINGLE_EVENT, event1), "The "
				+ event1 + " isn't correctly displayed on the month tab");
	}
	...
	public void testChooseDayTab() {
		...
		Assert.assertTrue(calendarPage.isEventPresent(
				CalendarPage.EventType.DAY_TAB_SINGLE_EVENT, event1), "The "
				+ event1 + " isn't correctly displayed on the day tab");
	}
	...
	public void testChooseWeekTab() {
		...
		Assert.assertTrue(calendarPage.isEventPresent(
				CalendarPage.EventType.WEEK_TAB_SINGLE_EVENT, event1), "The "
				+ event1 + " isn't correctly displayed on the week tab");
	}
	...
	public void testChooseAgendaTab() {
		...
		Assert.assertTrue(calendarPage.isEventPresent(
				CalendarPage.EventType.AGENDA_TAB_SINGLE_EVENT, event1), "The "
				+ event1 + " isn't correctly displayed on the agenda tab");

		int nrEvents = calendarPage
				.getTheNumOfEvents(ActionEventVia.AGENDA_TAB);
	...
	public void testShowWorkingHours() {
		...
		Assert.assertTrue(
				calendarPage
						.checkTableHours(CalendarPage.HoursFromTable.FIRST_WORKING_HOUR),
				"Not only working hours (7.00 - 18.00) are shown");
		Assert.assertTrue(
				calendarPage
						.checkTableHours(CalendarPage.HoursFromTable.LAST_WORKING_HOUR),
				"Not only working hours (7.00 - 18.00) are shown");
	}
	...
	public void testShowAllHours() {
		...
		Assert.assertTrue(calendarPage
				.checkTableHours(CalendarPage.HoursFromTable.FIRST_ALL_HOUR),
				"Not all hours are shown");
		Assert.assertTrue(calendarPage
				.checkTableHours(CalendarPage.HoursFromTable.LAST_ALL_HOUR),
				"Not all hours are shown");
	}

	@Test(dependsOnMethods = "testShowAllHours", timeOut = 60000)
	public void testIsShowAllItemsPresent() {
		Assert.assertTrue(calendarPage.isShowAllItemsPresent(),
				"Link 'Show All items' isn't displayed");
	}

	@Test(dependsOnMethods = "testIsShowAllItemsPresent", timeOut = 60000)
	public void testIsTagPresent() {
		Assert.assertTrue(calendarPage.isTagPresent(tag1), "Tag Link '" + tag1
				+ "' isn't displayed");
	}
	...
	public void testEditEvent() {
		...
		Assert.assertTrue(calendarPage.isEventPresent(
				CalendarPage.EventType.MONTH_TAB_SINGLE_EVENT, event1), "The "
				+ event1 + " isn't correctly displayed on the month tab");
		calendarPage = calendarPage.editEvent(event1,
				CalendarPage.EventType.MONTH_TAB_SINGLE_EVENT,
				CalendarPage.ActionEventVia.MONTH_TAB, edit_event1_what,
				edit_event1_where, edit_event1_description, null, null, null,
				null, null, false, null).render();
	}

	@Test(dependsOnMethods = "testEditEvent", timeOut = 60000)
	public void testCheckInformationEventForm() {
		InformationEventForm informationEventForm = calendarPage
				.clickOnEvent(CalendarPage.EventType.MONTH_TAB_SINGLE_EVENT,
						edit_event1_what).render();
		Assert.assertTrue(
				informationEventForm.getWhatDetail().contains(edit_event1_what),
				"The "
						+ edit_event1_what
						+ " isn't correctly displayed on the information form what field. Server B");
		Assert.assertTrue(
				informationEventForm.getWhereDetail().contains(
						edit_event1_where),
				"The "
						+ edit_event1_where
						+ " isn't correctly displayed on the information form where field. Server B");
		Assert.assertTrue(
				informationEventForm.getDescriptionDetail().contains(
						edit_event1_description),
				"The "
						+ edit_event1_description
	...
	public void testStartEndDateInfoFields() {
		...
		calendarPage = calendarPage.createEvent(
				CalendarPage.ActionEventVia.DAY_TAB, event_2, event_2, event_2,
				String.valueOf(todayDate), null, String.valueOf(todayDate),
				null, null, false);
		Assert.assertTrue(calendarPage.isEventPresent(
				CalendarPage.EventType.DAY_TAB_SINGLE_EVENT, event_2), "The "
				+ event_2 + " isn't correctly displayed on the day tab");

		InformationEventForm eventInfo = calendarPage.clickOnEvent(
				CalendarPage.EventType.DAY_TAB_SINGLE_EVENT, event_2).render();
		Assert.assertTrue(
				eventInfo.getWhatDetail().contains(event_2),
				"The "
						+ event_2
	...
	public void testCreateEventWithMonth() {
		...
		ArrayList<String> monthValues = new ArrayList<String>(Arrays.asList(
		...
		calendarPage = calendarPage.createEvent(
				CalendarPage.ActionEventVia.MONTH_TAB, event_month,
				event_month, event_month, String.valueOf(currentYear),
				String.valueOf(startMonth), String.valueOf(todayDate),
				"7:00 AM", String.valueOf(nextMonthsYear),
				String.valueOf(endMonth), String.valueOf(lastDate),
				"9:00 AM", null, false);
	

		// verify event is present
		Assert.assertTrue(calendarPage.isEventPresent(
				CalendarPage.EventType.MONTH_TAB_MULTIPLY_EVENT, event_month),
				"The " + event_month
		...
		InformationEventForm eventInfo = calendarPage.clickOnEvent(
				...
				.render();
		Assert.assertTrue(
				eventInfo.getWhatDetail().contains(event_month),
				"The "
						+ event_month
						+ " isn't correctly displayed on the information form what field.");
