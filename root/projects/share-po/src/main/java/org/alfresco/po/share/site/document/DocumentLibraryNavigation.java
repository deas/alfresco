        ...
        public DocumentLibraryPage selectDownloadAsZip()
        {
                if (AlfrescoVersion.Enterprise41.equals(alfrescoVersion) || alfrescoVersion.isCloud())
                {
                        throw new UnsupportedOperationException(
                                "Download as Zip option is not available on this version " + alfrescoVersion.toString());
                }
                clickSelectedItems();
                try
                {
                ...
                if (isNavigationBarVisible())
                {
                        ...
                        else
                        {
                                throw new PageOperationException(
                                        "You may be in the root folder, please check path and use folder up whenever required.");
                        }
                }
                else
                {
                        throw new PageOperationException(
                                "Navigation might be hidden, please click show breadcrump from option menu.");
                }
        }
