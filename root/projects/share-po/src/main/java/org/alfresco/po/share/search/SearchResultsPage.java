...
public abstract class SearchResultsPage extends SharePage 
{
    ...
    {
        return Pagination.selectPagiantionButton(drone, PAGINATION_BUTTON_NEXT);
    }
    ...
    {
        return Pagination.selectPagiantionButton(drone, PAGINATION_BUTTON_PREVIOUS);
    }
    ...
    {
        if (sortType == null) { throw new IllegalArgumentException("SortType can't be null."); }
        try
        {
            List<ShareLink> sortLinks = sortFilterList();
            for (ShareLink sortElement : sortLinks)
