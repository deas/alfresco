...
public class FileTransferPrimaryManifestProcessor extends AbstractManifestProcessorBase
{
    ...
    // this temporary location is used when receiving a node without parent
    ...
    protected void processNode(TransferManifestNormalNode node) throws TransferProcessingException
    {
        ...
        if (isNodeNew)
        {
            ...
            if (!isNodeUnderRoot)
            {
                ...
                // if no parent can be found then this is an error
            ...
            FileTransferInfoEntity newlyCereatednode = fTReceiver.findFileTransferInfoByNodeRef(node.getNodeRef()
                    .toString());
            // adjust path in DB
            adjustPathInSubtreeInDB(newlyCereatednode, parentPath + name + "/");
            if (log.isDebugEnabled())
            {
                log.debug("adjustPathInSubtreeInDB:" + newlyCereatednode);
