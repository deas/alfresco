...
//  private static int m_evalCount = 5;		//	demo
  ...
  private final static Vector BuildNegotiatePacket(SMBPacket pkt, DialectSelector dlct, int pid) {
   ...
   *
   * @exception SMBException If the evaluation has expired.
   */
  private static void CheckLicence()
  	throws SMBException {

    //  Check if this is an evaluation version, if so then check if we can create any more sessions

    if (m_evalCount >= 0) {

      //  Evaluation version, check if any more sessions can be created
  ...
  public final static boolean hasSessionDebug() {
	...
	public static AuthenticateSession OpenAuthenticateSession(PCShare shr, int tmo, DialectSelector dia)
		...
		Vector diaList = BuildNegotiatePacket(pkt, selDialect, pid);
