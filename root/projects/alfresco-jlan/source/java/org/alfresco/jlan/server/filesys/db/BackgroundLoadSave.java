			...
			while (mi_shutdown == false) {
				...
				if ( fileReq != null) {
					...
					// If the file request was not processed requeue it
						...
						// Check if we need to pause, if there are only a few requests in the
						// in-memory queue then we will
						// sleep for a short while so as not to process the requeued request again
						// too quickly
