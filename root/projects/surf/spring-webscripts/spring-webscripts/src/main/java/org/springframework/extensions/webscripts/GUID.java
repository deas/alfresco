...
/**
 * Generator for GUID values. Can use either a basic Random (default) or
 * SecureRandom source for time based MD5 random GUIDs.
 * <p>
...
public final class GUID
{
    ...
    static
    {
        ...
        try
        {
            unid = InetAddress.getLocalHost().toString();
        }
    ...
    {
        final StringBuilder beforeMD5 = new StringBuilder(64);
        beforeMD5.append(unid)
                 .append(':')
                 .append(Long.toString(System.currentTimeMillis()))
                 .append(':')
                 .append(Long.toString(secure ? mySecureRand.nextLong() : myRand.nextLong()));
        
        try
        {
            final MessageDigest md5 = MessageDigest.getInstance("MD5");
            md5.update(beforeMD5.toString().getBytes());
            
            final byte[] array = md5.digest();
            final StringBuilder sb = new StringBuilder(32);
            for (int j = 0; j < array.length; j++)
            {
                sb.append(hex[(array[j] & 0xFF)]);
            }
    ...
    {
         return new GUID(secure).toString();
    }
